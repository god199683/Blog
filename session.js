(() => {
  ...
  function getSessionExpiresAt(session) {
    return getJwtExpiresAt(session?.access_token) || Number(session?.expires_at || 0) || 0;
  }
  ...
  async function getFreshSession() {
    ...
    if (expiresAt && Number(session.expires_at || 0) !== expiresAt) {
      return writeSession({ ...session, expires_at: expiresAt });
    }
