...
function getPostLocationLabel(post = {}) {
  return post.folder_path || post.folder_name || post.folder || post.category || DEFAULT_CATEGORY;
}
...
function getPrimaryTrashPost(item = {}) {
  return (item.posts || []).find((post) => post.id) || null;
}

function getTrashItemViewHref(item = {}) {
  const post = getPrimaryTrashPost(item);
  return post ? getPostViewHref(post) : "";
}

function renderTrashPostLinks(item = {}) {
  const posts = (item.posts || []).filter((post) => post.id);
  if (posts.length === 0) return "";

  return `
    <div class="trash-page-posts" aria-label="삭제된 글">
      ${posts
        .map((post) => {
          ...
          const dateLabel = date && !Number.isNaN(date.getTime()) ? date.toLocaleDateString("ko-KR") : "";

          if (state.selectionMode) {
            return `
              <span class="trash-page-post-link is-static">
                <span>${escapeHtml(title)}</span>
                <small>${escapeHtml([location, dateLabel].filter(Boolean).join(" · "))}</small>
              </span>
            `;
          }

          return `
            <a class="trash-page-post-link" href="${getPostViewHref(post)}" data-trash-post-link>
              <span>${escapeHtml(title)}</span>
              <small>${escapeHtml([location, dateLabel].filter(Boolean).join(" · "))}</small>
            </a>
          `;
...
async function saveTreeDataToSupabase(data) {
  ...
  if (!response.ok) {
    ...
    throw new Error(data?.message || data?.hint || data?.details || "휴지통 변경 사항을 저장하지 못했습니다.");
  }
}
...
async function loadTreeData() {
  ...
  if (!hasTreeData(remote) && hasTreeData(local)) saveTreeDataToSupabase(local).catch(() => {});
...
async function saveTree() {
  ...
  await saveTreeDataToSupabase(data);
}
...
function render() {
  ...
  els.list.innerHTML = state.trashItems
    .map((item) => {
      ...
      const viewHref = getTrashItemViewHref(item);
      const viewAttribute = viewHref && !state.selectionMode ? ` data-trash-view-href="${escapeHtml(viewHref)}"` : "";
      ...
      return `
        <article class="trash-page-item ${isSelected ? "is-selected" : ""}" data-trash-item="${escapeHtml(item.id)}"${viewAttribute} tabindex="0">
          ${check}
          <div class="trash-page-item-main">
            <h2>${escapeHtml(item.label)}</h2>
            <p>${typeLabel} · ${countLabel}${dateLabel ? ` · ${dateLabel}` : ""}</p>
            ${renderTrashPostLinks(item)}
          </div>
        </article>
      `;
    })
...
function toggleTrashItemSelection(id) {
...
els.list.addEventListener("click", (event) => {
  ...
  if (state.selectionMode) {
    ...
    toggleTrashItemSelection(item.dataset.trashItem);
    return;
  }

  if (event.target.closest("a, button, input")) return;
  const viewHref = item.dataset.trashViewHref;
...
els.list.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Enter" && item.dataset.trashViewHref) {
    window.location.href = item.dataset.trashViewHref;
  }
});
